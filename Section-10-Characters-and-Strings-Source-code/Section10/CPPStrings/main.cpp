...
int main() {
    ...
    for (int i {1}; i< unformatted_full_name.length(); i++){
        if (isupper(unformatted_full_name[i]))
            temp = i;
    }
    cout << "Sub string pos: " << temp << endl;

    string first_name {unformatted_full_name, 0, temp};
    string last_name {unformatted_full_name.substr(temp,unformatted_full_name.length())};
    ...
    formatted_full_name.insert(temp, " ");
    cout << formatted_full_name << endl;

    string journal_entry_1 {"Isaac Newton"};
    string journal_entry_2 {"Leibniz"};

    size_t temp2 {journal_entry_1.find(" ")};
    if (temp2 != string::npos)
        journal_entry_1.erase(0,temp2+1);
